...
)
{
	...
	write_setup_inst.Stride              = write_setup_inst.HoriSizeInput;
	...
	for (ii = 0; ii < p_vdma_inst->MaxNumFrames; ii++)
	{
		...
		cur_fb_addr += write_setup_inst.HoriSizeInput * write_setup_inst.VertSizeInput;
	}
...
)
{
	...
	read_setup_inst.Stride              = read_setup_inst.HoriSizeInput;
	...
	for (ii = 0; ii < p_vdma_inst->MaxNumFrames; ii++)
	{
		...
		cur_fb_addr += read_setup_inst.HoriSizeInput * read_setup_inst.VertSizeInput;
	}
...
)
{
	...
	// Setup read channel
	status = set_vdma_read(p_vdma_inst, bits_per_pixel, hsize, vsize, buf_base_addr);
	if (status != XST_SUCCESS)
	{
	...
	// Setup write channel
	status = set_vdma_write(p_vdma_inst, bits_per_pixel, hsize, vsize, buf_base_addr);
	if (status != XST_SUCCESS)
	{
	...
	// Start write side
	status = XAxiVdma_DmaStart(p_vdma_inst, XAXIVDMA_WRITE);
	if (status != XST_SUCCESS)
	{
	...
	// Start read side
	status = XAxiVdma_DmaStart(p_vdma_inst, XAXIVDMA_READ);
	if (status != XST_SUCCESS)
	{
